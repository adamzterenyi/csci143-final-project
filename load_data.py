...
engine = create_engine("postgresql://postgres:pass@postgres:5432", echo=True, connect_args={
    'application_name': '__init__.py',
})

connection = engine.connect()

def insert_users(n_rows):
...
def insert_messages(n_rows, users_ids, urls_ids):
...
if __name__ == "__main__":
    ...
    # Insert data
    insert_users(n_rows)  # Insert n_rows users
    insert_urls(n_rows)  # Insert n_rows URLs

    # Fetch IDs
    users_ids = get_users_ids()
    urls_ids = get_urls_ids()

    # Insert messages
    insert_messages(n_rows, users_ids, urls_ids)
